...
func TestIgnoreError(t *testing.T) {
	fs := NewFlagSet("ignore errors test", IgnoreError)
	definedA := fs.Bool("definedA", false, "defined flag A")
	definedC := fs.String("definedC", "C", "defined flag C")
	var buf bytes.Buffer
	fs.SetOutput(&buf)
	fs.Parse([]string{"--definedA=true", "--undefinedB", "B", "-definedC", "C", "more", "args"})
	args := fs.Args()
	if !*definedA || *definedC != "C" || len(args) != 4 ||
		args[0] != "--undefinedB" || args[1] != "B" ||
		args[2] != "more" || args[3] != "args" {
		t.Fatalf("Wrong parsing when errors are ignored. definedA=%v, definedC=%v, args=%v",
			*definedA, *definedC, args)
	}
	got := buf.String()
	if got != "" {
